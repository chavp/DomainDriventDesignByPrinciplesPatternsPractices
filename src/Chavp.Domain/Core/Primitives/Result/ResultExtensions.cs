...
namespace Chavp.Domain.Core.Primitives.Result
{
    /// <summary>
    ...
    public static class ResultExtensions
    {
        ...
        /// </returns>
        public static Result<T> Ensure<T>(this Result<T> result, Func<T, bool> predicate, List<Error> errors)
        {
            if (result.IsFailure)
            {
                return result;
        ...
        public static Result<T> Validate<T>(this Result<T> result, IValidator<T> validator)
        {
            ...
            if (!validate.IsValid)
            {
                ...
                {
                    errors.Add(new Error(err.ErrorCode, err.ErrorMessage));
                }
            }

            return result.IsSuccess && validate.IsValid ? result : Result.Failure<T>(errors);
        }
